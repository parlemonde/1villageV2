...
export default function CreerUnJeuMimiqueStep2() {
    const t = useExtracted('app.(1village).(activities).creer-un-jeu.mimique.2');
    ...
    return (
        <PageContainer>
            ...
            <GestureForm
                ...
                number={2}
                stepId={2}
                title={t('Présentez en vidéo une 2ème mimique à vos pélicopains')}
            />
            <div className={styles.buttonsContainer}>
                <Button as="a" href="/creer-un-jeu/mimique/1" color="primary" label={tCommon('Étape precedente')} leftIcon={<ChevronLeftIcon />} />
                <Button
                    disabled={!GESTURE_GAME_STEPS_VALIDATORS.isStep2Valid(activity)}
                    label={tCommon('Étape suivante')}
                    rightIcon={<ChevronRightIcon />}
                    color="primary"
                    onClick={() => router.push('/creer-un-jeu/mimique/3')}
                />
            </div>
        </PageContainer>
    );
